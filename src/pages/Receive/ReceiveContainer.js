...
const ReceiveContainer = () => {
  ...
  console.info("myStream : ", myStream);
  //
  ...
  const initCall = async () => {
    ...
    await makeConnection(); // socket.current.io 연결시 필수 코드
    // console.log("연결됨");
  };
  ...
  useEffect(() => {
    ...
    socket.current.on("offer", async (offer) => {
      console.log("offer:", offer);
      await myPeerConnection.setRemoteDescription(offer); // 다른 peer의 위치를 myPeerConnection에 연결해주는 과정
      const answer = await myPeerConnection.createAnswer(); //잘 받았음을 확인하기 위해 Answer 만듦
      await myPeerConnection.setLocalDescription(answer); //현재 peer에서 생성한 answer을 myPeerConnection의 LocalDescription으로 등록
